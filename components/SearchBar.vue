    ...
    <div class="grid grid-cols-2 lg:flex lg:flex-row gap-7 items-center justify-center">
      ...
      <div class=" flex-1">
        <InputGroup>
          ...
          <FloatLabel>
            <Calendar v-model="in_out" selectionMode="range" dateFormat="dd/mm/yy" class="custom-calendar" />
      ...
      <div>
        <Button type="button" label="Search" icon="pi pi-search" class="bg-primary-normal" :loading="loading"
          @click="load" />
      </div>
    </div>
  </div>
...
const search = () => {
  const searchData = {
    checkin: formatDate(in_out.value[0]),
    checkout: formatDate(in_out.value[1]),
    adults: adults.value,
