    ...
    <div>
        <div v-if="!loading" class="container">
            <section
                ...
                <img :src="`${BACKEND_URL}${property?.Images[0].url}`"
                    class="h-full w-full" />
                <div
            ...
            <section class="grid grid-cols-5 gap-12 mt-8">
                <section
                    class="flex flex-col gap-8 col-span-3">
            ...
            $t('variables.night') }}</span>
                    ...
                    <div
                        v-html="getPropertyText(property)?.description">
                    </div>

                    <section
                        class="grid grid-cols-2 md:grid-cols-6 gap-4">
                        <div v-if="property.wifi"
                        ...
                        <div v-if="property.swimming_pool"
                            class="text-2xl border bg-gray-300 p-4 flex justify-center items-center">
                            <img src="../../assets/images/MaterialSymbolsPool.svg"
                                width="24" />
                        </div>
                        <div v-if="property.terrace"
                            class="border bg-gray-300 p-4 flex justify-center items-center">
                            <img src="../../assets/images/CbiRoomsbalcony.svg"
                                width="24" />
                        </div>
                    </section>
                </section>
                        ...
                        <FloatLabel>
                            <Calendar v-model="data.dates"
                                ...
                                <template #date="slotProps">
                                    <strong v-if="reservedDates.includes(
            slotProps.date.year +
            '-' +
        ...
        <BookDialog v-if="!loading"
            :visible="modalData.visible" :data="data"
            :property="property" :loading="loading"
...
onMounted(async () => {
    const data = await fetchProperty(useRoute().params?.id);
    property.value = data.property;

    if (data.reservedDates) {
        reservedDates.value = data.reservedDates.map((date) =>
